...
static char *acpi_printf_buffer;
...
int acpi_init(void) {
	...
	acpi_printf_buffer = (char *) kmalloc(1024);

	// ACPICA subsystem
	status = AcpiInitializeSubsystem();
	if (ACPI_FAILURE(status)) {
		KERROR("ACPICA init failed (%i)\n", status);
		return status;
	}
	...
	if (ACPI_FAILURE(status)) {
		KERROR("Table manager initialisation failed (%i)\n", status);
		return status;
	}
	...
	if (ACPI_FAILURE(status)) {
		KERROR("Namespace initialisation failed (%i)\n", status);
		return status;
	}
	...
	if (ACPI_FAILURE(status)) {
		KERROR("ACPI subsystem enable failed (%i)\n", status);
		return status;
	}
	...
	if (ACPI_FAILURE(status)) {
		KERROR("Object initialisation failed (%i)\n", status);
		return status;
	}*/

	KSUCCESS("ACPI initialised");
...
void *AcpiOsMapMemory(ACPI_PHYSICAL_ADDRESS PhysicalAddress, ACPI_SIZE length) {
	...
	uint32_t location = paging_map_section(PhysicalAddress, (unsigned int) length, kernel_directory, kMemorySectionHardware);
...
void AcpiOsUnmapMemory(void *where, ACPI_SIZE length) {
	...
	if((unsigned int) where <= 0x00100000) {
		return;
	}

	// kprintf("ACPI: Unmapped virtual 0x%X (len = 0x%X)\n", (uint32_t) where, length);
	paging_unmap_section((unsigned int) where, length, kernel_directory);
}
...
void *AcpiOsAllocate(ACPI_SIZE size) {
	// Ensure size is a multiple of four
	if(size & 0x03) {
		size += 4 - (size & 0x03);
	}
...
ACPI_STATUS AcpiOsInstallInterruptHandler(UINT32 InterruptLevel, ACPI_OSD_HANDLER Handler, void *Context) {
	kprintf("ACPI: Register IRQ handler for level %i at 0x%X\n", InterruptLevel, (unsigned int) Handler);
	hal_register_irq_handler(InterruptLevel, (hal_irq_callback_t) Handler, Context);
...
void AcpiOsPrintf(const char *format, ...) {
	char* buffer = (char *) acpi_printf_buffer;
...
void AcpiOsVprintf(const char *format, va_list va) {
	char* buffer = (char *) acpi_printf_buffer;
	int n = vsprintf(buffer, format, va);
...
ACPI_STATUS AcpiOsSignal(UINT32 Function, void *Info) {
	if(Function == ACPI_SIGNAL_FATAL) {
		...
		KWARNING("Encountered ACPI Signal %i (0x%X)\n", Function, (unsigned int) Info);
	}
